...
app.get('/data/age', (req, res) => {
  fs.readFile('data.json', 'utf8', (err, data) => {
    ...
    const jsonData = data ? JSON.parse(data) : [];
    const filteredData = jsonData.filter(data => data.age > 20);

    filteredData.forEach(data => {
      console.log(`Nom : ${data.name}, Âge : ${data.age}, Langages : ${data.languages.join(', ')}`);
    });

    res.render('data', { data: filteredData });
  });
});
app.get('/data/java', (req, res) => {
  fs.readFile('data.json', 'utf8', (err, data) => {
    if (err) {
      console.error(err);
      res.status(500).send('Internal Server Error');
    ...
    const jsonData = data ? JSON.parse(data) : [];
    const filteredData = jsonData.filter(data => data.languages.includes("Java"));
